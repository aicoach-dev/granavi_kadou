...
フォーマット: CSV（UTF-8 BOM対応）、列: name, company, team
マッチキー : (name, company) — どちらも必須
...
def read_team_csv(bucket: str, key: str) -> dict:
    """
    S3 からチームCSVを読み込み (name, company) → team の辞書を返す。
...
def apply_team_updates(
    ...
    for rec in records:
        ...
        team = team_map.get((name, company))
        if team is None:
            counts["team_no_match"] += 1
            logger.debug(f"team マッチなし: {name}/{company}")
            continue
        ...
        try:
            ...
            logger.debug(f"team 更新: {name}/{company} → {team}")
        except ClientError as e:
