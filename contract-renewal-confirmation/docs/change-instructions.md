...
### 契約期間表示・緊急停止アイコンのUX改善（依頼：2026-08-03）

バックログに記録されていた2件のUX課題を解消する、独立した小規模タスク。Round6（送信フロー・SES連携）とは無関係、スキーマ変更なし。

**調査完了（2026-08-03）**：
- 契約期間表示について、`git status`・`git stash list`・`git branch -a`・`git reflog`による調査の結果、periodStart/periodEndをHTML上に表示するコードは、このリポジトリのどこにも（コミット・stash・他ブランチ・reflogいずれにも）存在しないことを確認した。バックログ記載の「6/21-9:20」という表記の出処は特定できなかったが、`getCandidates.ts`がperiodStart/periodEndを既にAPIレスポンスに含めている（未使用）ことは確認済み。
- 緊急停止アイコンのマークアップ（`frontend/ops-console.html`、`.est-on`/`.est-off`クラスによるCSS color指定、絵文字`🚨`をtextContentで設定）を確認済み。

**UX相談スレッドでの再検討の結果、対応内容を以下に確定する（`docs/requirements.md` 4.11・7.4も合わせて更新済み）**：

1. **契約期間の表示**：新規列としては追加しない。既存の「客先」列を2行表示にし、1行目に客先名、2行目に対象期間をサブテキストとして表示する（「氏名・チーム」列の2行表示パターンを踏襲）。
   - フォーマット：「2026/6/21〜9/20」（開始日側にのみ年を表示、終了日側は月日のみ、月日ゼロ埋めなし、区切りは全角チルダ「〜」）
2. **一覧のデフォルトソート順の変更**：氏名順から、契約開始日の近い順（昇順）に変更する。
3. **緊急停止アイコンのON/OFF表示修正**：🚨絵文字へのCSS colorが効かない問題を修正する。
   - アイコン自体（絵文字）は変更しない。
   - ON（緊急停止中）の場合のみ、アイコンに隣接してテキストラベル「停止中」を表示する。
   - OFF（通常稼働中）の場合はラベルを表示しない（アイコンのみ）。
   - 色は既存のdanger系のスタイル・クラスがあればそれに合わせ、新規に色を定義しない。

推奨コミット単位：
1. ドキュメント更新（decisions-log.md・change-instructions.md・requirements.md）※本タスクで実施
2. 契約期間の客先列統合・デフォルトソート順変更
3. 緊急停止アイコンのラベル追加

このサブセクションの「着手前の確認事項」は完了したため、次の着手時は上記の実装内容（1〜3）にそのまま進んでよい。
