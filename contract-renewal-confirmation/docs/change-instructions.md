...
対応内容：
1. **契約期間の表示形式修正**：一覧の期間列が「6/21-9:20」のような意図しない表記になる不具合（2026-08-01バックログ記載）を修正する。
   - 目標フォーマット：「2026/6/21 〜 9/20」（開始日側にのみ年を表示、終了日側は月日のみ、月日ゼロ埋めなし、区切りは全角チルダ「〜」）
   - この不具合は表示調整ではなく、日付/時刻フォーマット関数の取り違え等の実装上の原因が疑われる。まず現状のコード（期間文字列を生成している箇所）を特定し、原因を報告してから修正すること。原因を推測したまま先に修正を当てない。
2. **緊急停止アイコンのON/OFF表示修正**：🚨絵文字へのCSS colorが効かない問題（2026-08-01バックログ記載）を修正する。
   - アイコン自体（絵文字）は変更しない。
...
着手前の確認事項（このタスク固有）：
- 期間列の文字列生成箇所（ファイル・関数）を特定し、報告する。
- 緊急停止アイコンの現在のマークアップ（絵文字+CSS colorの実装箇所）を特定し、報告する。
- 上記2点の報告後、実装に進んでよい。

推奨コミット単位：
1. ドキュメント更新（decisions-log.md・change-instructions.md）
2. 契約期間表示の修正
3. 緊急停止アイコンのラベル追加
