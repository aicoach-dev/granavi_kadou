...
- 是正（2026-07-31）：上記の①を含め、Bash形式47件のみでは主シェル（PowerShell）に効いていなかったことが判明。同じコマンド群をPowerShell形式でも追加し、合計94件（Bash形式47件＋PowerShell形式47件）に更新（コミット `7f07a98` の是正版として同日コミット）。
