...
- 2026-08-03：ops-console.htmlの表示改善2件（契約期間フォーマット・緊急停止アイコンのラベル追加）をRound外タスクとして起票。
- 2026-08-03：契約期間表示をUX相談スレッドでゼロベース再検討し、客先列への統合・デフォルトソート順変更に決定。requirements.md 4.11・7.4を初めてCodeが編集する対象として指示。
- 2026-08-03：フロントエンドのS3デプロイがIaCを経由せず直接AWS CLIコマンドで行われていたことが判明。IaC化(BucketDeployment)を優先し、保留中のUX変更2コミットの反映はその後とする方針を決定。
- 2026-08-04：フロントエンドデプロイのIaC化（BucketDeployment）が完了。Phase 1〜2のトラブルシューティングを経て、契約期間表示・緊急停止アイコンのUX変更が本番環境（CloudFront）に反映されたことを3者ETag一致で確認。
...
### 契約期間表示・緊急停止アイコンのUX改善（依頼：2026-08-03、完了：2026-08-04）

バックログに記録されていた2件のUX課題を解消した。Round6（送信フロー・SES連携）とは無関係、スキーマ変更なし。

**対応内容**：
1. **契約期間の表示**：新規列としては追加せず、既存の「客先」列を2行表示にし、1行目に客先名、2行目に対象期間をサブテキストとして表示（フォーマット：「2026/6/21〜9/20」、開始日側にのみ年を表示）。
2. **一覧のデフォルトソート順の変更**：氏名順から、契約開始日の近い順（昇順）に変更。
3. **緊急停止アイコンのON/OFF表示修正**：ON（緊急停止中）の場合のみ、アイコンに隣接してテキストラベル「停止中」を表示するよう修正。

実装：コミット`d7d015b`（客先列統合・ソート変更）・`d897b15`（緊急停止ラベル追加）。`docs/requirements.md` 4.11・7.4も合わせて更新済み。

**本番反映**：下記「フロントエンドデプロイのIaC化」タスクの完了により、2026-08-04にCloudFront経由で本番反映を確認済み（ローカルMD5・S3 ETag・CloudFront ETagの3者一致）。ユーザーによるブラウザでの目視確認待ち。

### フロントエンドデプロイのIaC化（依頼：2026-08-03、完了：2026-08-04）

frontend/のS3デプロイが、CLAUDE.md絶対ルール2（AWS CLIコマンドを直書きしない）に反し、直接AWS CLIコマンドで行われていたことが判明したため、CDKの`BucketDeployment`によるIaC化を実施した。

**実施内容**：
- `frontend/index.html`をgit管理下に追加（S3上にのみ存在しgit管理外だったプレースホルダー）。
- `infra/lib/infra-stack.ts`に`BucketDeployment`を追加。Phase 1〜1c（PermissionsBoundaryへの`AllowCDKBucketDeploymentLogs`・`AllowCDKAssetsBucketRead`・`cloudfront:GetInvalidation`の追加）を経て、Phase 2（BucketDeployment有効化）まで完了。トラブルシューティングの詳細経緯は`docs/decisions-log.md`の2026-08-04付エントリを参照。
- CDK bootstrapバケット共有によるアクセス範囲拡大（他プロジェクトのビルド成果物への読み取り）は、残存リスクとして受け入れ済み（2026-08-04決定）。将来的な専用bootstrap qualifierの新設は将来のための申し送り（バックログ）に追加済み。
- 実装コミット：`a917d8c`（PermissionsBoundary 3件修正・BucketDeployment有効化）。

**確認済み**：デプロイ成功（ロールバックなし）、CloudFrontキャッシュ無効化の自動実行、ローカルMD5・S3 ETag・CloudFront ETagの3者一致。

**今後への効果**：以後のフロントエンド変更は`cdk deploy`のみでS3同期・CloudFrontキャッシュ無効化まで自動化される。
...
- **CDK bootstrap qualifierの専用化**：現在、ContractRenewalStackは他のCDKスタックと同じデフォルトのbootstrap qualifier(`hnb659fds`)・bootstrapバケットを共有している。BucketDeploymentのLambdaロールが、このバケット全体(他プロジェクトのビルド成果物を含む)への読み取り権限を持つ状態になっている(2026-08-04決定、残存リスクとして受け入れ済み)。将来、より厳密な分離が必要になった場合、`cdk bootstrap --qualifier`によるこのプロジェクト専用のbootstrapインフラ(バケット・IAMロール等)の新設を検討する。AWS Organizations移行の検討（既存バックログ項目）とあわせて再検討する余地がある。
- **`.gitattributes`によるCRLF/LF問題の解決**：Windows環境の`core.autocrlf=true`により、`cdk deploy`のたびにPython Lambda(週次同期)のコードハッシュが変化し、内容に変更がなくても再デプロイが発生する（2026-08-03〜04判明）。`*.py text eol=lf`等の追加で解消できる（工数：数分、影響範囲：infra/lambda関連のみ）。
